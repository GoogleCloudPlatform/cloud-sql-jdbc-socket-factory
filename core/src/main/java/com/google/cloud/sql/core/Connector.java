...
class Connector {
  ...
  ConnectionInfoCache getConnection(final ConnectionConfig config) {
    ...
    String unresolvedName = ! Strings.isNullOrEmpty(config.getDomainName()) ? config.getDomainName() : config.getCloudSqlInstance();

    try {
      Function<String, String> resolver = config.getConnectorConfig().getInstanceNameResolver();
      if(resolver != null) {
        name = new CloudSqlInstanceName(resolver.apply(unresolvedName));
      } else {
    ...
    final ConnectionConfig updatedConfig = config
        .withCloudSqlInstance(name.getConnectionName());
